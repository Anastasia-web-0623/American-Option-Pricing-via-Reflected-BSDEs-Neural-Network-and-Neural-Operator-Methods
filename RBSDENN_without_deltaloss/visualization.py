...
# Function to generate and save various plots
def plot_and_save(results_df: pd.DataFrame, outdir: str):
    """Generates and saves a series of evaluation plots."""

    # 1) Premium in different groups (norm)
    ...
    # 2) Terminal MSE in different groups
    ...
    # 3) Avg Push in different groups
    ...
    # 5) American Price vs Black–Scholes (absolute)
    plt.figure()
    ...
    if 'Am_Delta' in results_df.columns and 'BS_Delta' in results_df.columns:
        ...
        plt.xlabel('BS Delta')
        plt.ylabel('American Delta (Z/(σS))')
        plt.savefig(os.path.join(outdir, 'delta_vs_bs.png'), bbox_inches='tight')
        plt.close()

    # 7) Premium (Absolute) in different groups
